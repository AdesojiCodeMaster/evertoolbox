// universal-filetool.js — glue between UI and core-modules
...
drop.addEventListener('dragover', e => { e.preventDefault(); drop.style.borderColor = '#0b69ff' });
drop.addEventListener('dragleave', () => { drop.style.borderColor = '#d6d9e6' });
drop.addEventListener('drop', async e => { e.preventDefault(); drop.style.borderColor = '#d6d9e6'; const f = e.dataTransfer.files[0]; await setFile(f); });
...
async function setFile(f){
  ...
  filespec.textContent = `Detected extension: ${core.extFromName(f.name)}`;
  bar.style.width = '0%';
}
...
startBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if(cat === 'image'){
      ...
      const crf = Math.max(18, Math.round(32 - (q * 14)));
    ...
    if(outFile){
      ...
      info.textContent = 'Processing finished (download triggered).';
    }
  } catch(err){
    console.error(err);
